...
bool op_06(Survivor* survivor, uint16_t shared_memory) {
    ...
    survivor->SP -= 2;
    uint16_t adress = survivor->SP;

    uint16_t segment = ((adress+0x10*survivor->SS) & 0xF0000) >> 16;
    if (segment != 0 && segment != survivor->stack_id && segment != shared_memory) {return false;}

    ((char*)memory)[(uint32_t) adress + 0x10*survivor->SS] = (survivor->ES & 0xFF00) >> 8;

    adress--;
    ((char*)memory)[(uint32_t) adress + 0x10*survivor->SS] += survivor->ES & 0xFF;
